...
// +------------------------------------------------------------------+
// | LUMORA 30-Second Signal Bridge                                  |
// | External signal publishing only - does NOT change EA strategy.  |
// +------------------------------------------------------------------+
input group "🌐 LUMORA 30-Second Signal Bridge"
input bool EnableLumoraSignals = false;                  // Publish signals to LUMORA dashboard
input string LumoraApiURL = "https://YOUR-DOMAIN.com/api/v1/signals"; // LUMORA endpoint
input string LumoraApiKey = "";                          // Optional API key
input int LumoraSignalIntervalSeconds = 30;              // Signal cycle
input int LumoraExpirySeconds = 30;                       // Binary signal expiry
input double LumoraMinSignalScore = 5.0;                  // Minimum signal score
input double LumoraStakeUSD = 2.0;                        // Dashboard stake per signal
input double LumoraPayoutPercent = 80.0;                  // WIN payout percentage
input bool LumoraSendNeutralSignals = false;              // Publish WAIT events
input int LumoraHttpTimeoutMs = 5000;                     // HTTP timeout
input int LumoraMarketIntervalSeconds = 1;               // Live MT5 market update interval
input int LumoraChartBars = 60;                           // M1 candles sent to dashboard

// +------------------------------------------------------------------+
// | LUMORA Live MT5 Market Snapshot                                  |
// | Sends current quote/indicators plus recent M1 OHLC candles.      |
// | External publishing only; no strategy/trade execution changes.  |
// +------------------------------------------------------------------+
// | Global Variables                                                 |
// +------------------------------------------------------------------+
...
// LUMORA signal bridge state
int lumoraSequence = 0;
datetime lumoraLastSignalTime = 0;
int lumoraPendingDirection = 0;                       // 1=BUY, -1=SELL, 0=none
datetime lumoraPendingExpiry = 0;
double lumoraPendingEntry = 0.0;
string lumoraPendingId = "";
datetime lumoraLastMarketTime = 0;
...
int InitializeEA()
{
    ...
    // LUMORA bridge uses a 1-second timer for the 30-second signal cycle.
    // This is external publishing only; it does not modify the EA entry logic.
    if(EnableLumoraSignals)
    {
        EventSetTimer(1);
        Print("LUMORA 30-second signal bridge enabled. Endpoint: ", LumoraApiURL);
    }
...
{
    if(EnableLumoraSignals) EventKillTimer();
...
// | External publishing only; no strategy/trade execution changes.  |
// +------------------------------------------------------------------+
void PublishLumoraMarket()
{
    if(!EnableLumoraSignals || LumoraApiURL == "") return;

    int bars = LumoraChartBars;
    if(bars < 10) bars = 10;
    if(bars > 120) bars = 120;

    // Read bid/ask from one synchronized tick so the displayed spread is truly live.
    MqlTick tick;
    ...
    double atrValue=0.0, emaFastValue=0.0, emaSlowValue=0.0, rsiValue=0.0;
    double b1[1];
    if(CopyBuffer(atrSignalHandle,0,0,1,b1)>0) atrValue=b1[0];
    if(CopyBuffer(emaFastHandle,0,0,1,b1)>0) emaFastValue=b1[0];
    if(CopyBuffer(emaSlowHandle,0,0,1,b1)>0) emaSlowValue=b1[0];
    if(CopyBuffer(rsiHandle,0,0,1,b1)>0) rsiValue=b1[0];

    string payload="{";
    payload += "\"symbol\":\""+LumoraJsonEscape(_Symbol)+"\",";
    payload += "\"timeframe\":\"M1\",";
    payload += "\"server_time\":\""+LumoraJsonEscape(LumoraTimeText(TimeCurrent()))+"\",";
    payload += "\"server_epoch\":"+IntegerToString((int)TimeCurrent())+",";
    ...
    payload += "\"spread_points\":"+DoubleToString((ask-bid)/_Point,1)+",";
    payload += "\"tick_time_msc\":"+IntegerToString((int)tick.time_msc)+",";
    payload += "\"atr\":"+DoubleToString(atrValue,_Digits)+",";
    payload += "\"ema_fast\":"+DoubleToString(emaFastValue,_Digits)+",";
    payload += "\"ema_slow\":"+DoubleToString(emaSlowValue,_Digits)+",";
    payload += "\"rsi\":"+DoubleToString(rsiValue,2)+",";
    payload += "\"candles\":[";
    for(int i=bars-1;i>=0;i--)
    {
        datetime t=iTime(_Symbol,PERIOD_M1,i);
        double o=iOpen(_Symbol,PERIOD_M1,i), h=iHigh(_Symbol,PERIOD_M1,i), l=iLow(_Symbol,PERIOD_M1,i), c=iClose(_Symbol,PERIOD_M1,i);
        long v=iVolume(_Symbol,PERIOD_M1,i);
        if(i<bars-1) payload += ",";
        payload += "{\"time\":"+IntegerToString((int)t)+",\"open\":"+DoubleToString(o,_Digits)+",\"high\":"+DoubleToString(h,_Digits)+",\"low\":"+DoubleToString(l,_Digits)+",\"close\":"+DoubleToString(c,_Digits)+",\"volume\":"+IntegerToString((int)v)+"}";
    }
    payload += "]}";
    LumoraPost("market",payload);
    lumoraLastMarketTime=TimeCurrent();
}

// +------------------------------------------------------------------+
// | LUMORA 30-Second Signal Bridge                                  |
// | Reads the existing signal engine and publishes signals only.    |
// | It never calls OpenPosition() and therefore does not alter EA   |
// | execution/trading logic.                                       |
// +------------------------------------------------------------------+
void OnTimer()
{
    if(!EnableLumoraSignals || !passwordVerified) return;
    if(LumoraApiURL == "" || StringFind(LumoraApiURL, "YOUR-DOMAIN") >= 0) return;

    datetime now = TimeCurrent();

    int marketInterval = LumoraMarketIntervalSeconds;
    if(marketInterval < 1) marketInterval = 1;
    if(lumoraLastMarketTime == 0 || (now - lumoraLastMarketTime) >= marketInterval)
        PublishLumoraMarket();

    // Settle the previous 30-second signal when its expiry arrives.
    if(lumoraPendingDirection != 0 && lumoraPendingExpiry > 0 && now >= lumoraPendingExpiry)
        PublishLumoraResult();

    int interval = LumoraSignalIntervalSeconds;
    if(interval < 1) interval = 30;
    if(lumoraLastSignalTime > 0 && (now - lumoraLastSignalTime) < interval) return;

    PublishLumoraSignal();
}

string LumoraJsonEscape(string value)
{
    StringReplace(value, "\\", "\\\\");
    StringReplace(value, "\"", "\\\"");
...
string LumoraTimeText(datetime t)
{
    if(t <= 0) return "";
    return TimeToString(t, TIME_DATE|TIME_SECONDS);
}

bool LumoraPost(string eventType, string payload)
{
    if(!EnableLumoraSignals || LumoraApiURL == "") return false;

    string body = "{";
    body += "\"event\":\"" + LumoraJsonEscape(eventType) + "\",";
    body += "\"source\":\"Nyao Scalper v43.0\",";
    body += "\"symbol\":\"" + LumoraJsonEscape(_Symbol) + "\",";
    body += "\"timeframe\":\"" + LumoraJsonEscape(EnumToString(_Period)) + "\",";
    body += "\"server_time\":\"" + LumoraJsonEscape(LumoraTimeText(TimeCurrent())) + "\",";
    body += "\"data\":" + payload;
    body += "}";
    ...
    if(LumoraApiKey != "")
        headers += "Authorization: Bearer " + LumoraApiKey + "\r\n";
    ...
    ResetLastError();
    int res = WebRequest("POST", LumoraApiURL, headers, LumoraHttpTimeoutMs,
                         post, result, resultHeaders);

    if(res == 200 || res == 201 || res == 202 || res == 204)
    {
        LogPrint("[LUMORA] ", eventType, " published. HTTP=", res);
        return true;
    }

    LogPrint("[LUMORA] HTTP ERROR=", res, " | Error=", GetLastError());
    LogPrint("[LUMORA] Response=", CharArrayToString(result));
    return false;
}

void PublishLumoraSignal()
{
    // Use the EA's existing BuySignal()/SellSignal() decision functions.
    // These functions perform the same entry checks/threshold logic as the
    // normal EA path, but they only RETURN a score here; they do not place an order.
    double buySignal = BuySignal();
    double sellSignal = SellSignal();

    double threshold = LumoraMinSignalScore;
    ...
    {
        score = MathMax(buySignal, sellSignal);
        if(!LumoraSendNeutralSignals)
        {
            lumoraLastSignalTime = TimeCurrent();
            return;
        }
    }
    ...
    lumoraSequence++;
    string signalId = StringFormat("LUM-%d-%d", (int)TimeCurrent(), lumoraSequence);

    int expiry = LumoraExpirySeconds;
    if(expiry < 1) expiry = 30;
    datetime expiryTime = TimeCurrent() + expiry;

    MqlTick signalTick;
    if(!SymbolInfoTick(_Symbol, signalTick)) return;
    double bid = signalTick.bid;
    double ask = signalTick.ask;
    double spread = (ask > 0 && bid > 0) ? (ask - bid) : 0.0;

    string reasoning = "No qualifying signal";
    if(direction == 1) reasoning = "EA BUY signal passed all current entry checks";
    if(direction == -1) reasoning = "EA SELL signal passed all current entry checks";

    string payload = "{";
    payload += "\"signal_id\":\"" + LumoraJsonEscape(signalId) + "\",";
    payload += "\"side\":\"" + side + "\",";
    payload += "\"status\":\"ACTIVE\",";
    payload += "\"signal_epoch\":" + IntegerToString((int)TimeCurrent()) + ",";
    payload += "\"entry_price\":" + DoubleToString(price, _Digits) + ",";
    payload += "\"expiry_seconds\":" + IntegerToString(expiry) + ",";
    payload += "\"expiry_epoch\":" + IntegerToString((int)expiryTime) + ",";
    payload += "\"expiry_time\":\"" + LumoraJsonEscape(LumoraTimeText(expiryTime)) + "\",";
    payload += "\"score\":" + DoubleToString(score, 2) + ",";
    payload += "\"confidence\":" + DoubleToString(MathMin(100.0, score * 10.0), 1) + ",";
    payload += "\"buy_score\":" + DoubleToString(buySignal, 2) + ",";
    payload += "\"sell_score\":" + DoubleToString(sellSignal, 2) + ",";
    payload += "\"bid\":" + DoubleToString(bid, _Digits) + ",";
    payload += "\"ask\":" + DoubleToString(ask, _Digits) + ",";
    payload += "\"spread_points\":" + DoubleToString(spread/_Point, 1) + ",";
    payload += "\"tick_time_msc\":" + IntegerToString((int)signalTick.time_msc) + ",";
    double atrValue = 0.0;
    double emaFastValue = 0.0;
    double emaSlowValue = 0.0;
    double rsiValue = 0.0;
    double atrBuf[1];
    double emaFastBuf[1];
    double emaSlowBuf[1];
    double rsiBuf[1];
    if(CopyBuffer(atrSignalHandle, 0, 0, 1, atrBuf) > 0) atrValue = atrBuf[0];
    if(CopyBuffer(emaFastHandle, 0, 0, 1, emaFastBuf) > 0) emaFastValue = emaFastBuf[0];
    if(CopyBuffer(emaSlowHandle, 0, 0, 1, emaSlowBuf) > 0) emaSlowValue = emaSlowBuf[0];
    if(CopyBuffer(rsiHandle, 0, 0, 1, rsiBuf) > 0) rsiValue = rsiBuf[0];

    payload += "\"atr\":" + DoubleToString(atrValue, _Digits) + ",";
    payload += "\"ema_fast\":" + DoubleToString(emaFastValue, _Digits) + ",";
    payload += "\"ema_slow\":" + DoubleToString(emaSlowValue, _Digits) + ",";
    payload += "\"rsi\":" + DoubleToString(rsiValue, 2) + ",";
    payload += "\"reasoning\":\"" + LumoraJsonEscape(reasoning) + "\"";
    payload += "}";

    bool sent = LumoraPost(direction == 0 ? "signal_wait" : "signal", payload);
    lumoraLastSignalTime = TimeCurrent();

    if(sent && direction != 0)
    {
        lumoraPendingDirection = direction;
        lumoraPendingEntry = price;
        lumoraPendingExpiry = expiryTime;
        lumoraPendingId = signalId;
    }
}

void PublishLumoraResult()
{
    if(lumoraPendingDirection == 0 || lumoraPendingEntry <= 0 || lumoraPendingId == "")
    {
        lumoraPendingDirection = 0;
        lumoraPendingExpiry = 0;
        lumoraPendingEntry = 0;
        lumoraPendingId = "";
        return;
    }

    // Result is measured from the same MT5/broker quote feed used by the EA.
    double expiryPrice = (lumoraPendingDirection == 1)
                       ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                       : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(expiryPrice <= 0) return;

    bool win = (lumoraPendingDirection == 1)
             ? (expiryPrice > lumoraPendingEntry)
             : (expiryPrice < lumoraPendingEntry);

    double stake = LumoraStakeUSD;
    if(stake < 0) stake = 0;
    double payout = LumoraPayoutPercent;
    if(payout < 0) payout = 0;

    double resultPL = win ? (stake * payout / 100.0) : -stake;
    string result = win ? "WIN" : "LOSS";
    string side = (lumoraPendingDirection == 1) ? "BUY" : "SELL";

    string payload = "{";
    payload += "\"signal_id\":\"" + LumoraJsonEscape(lumoraPendingId) + "\",";
    payload += "\"side\":\"" + side + "\",";
    payload += "\"status\":\"CLOSED\",";
    payload += "\"result\":\"" + result + "\",";
    payload += "\"entry_price\":" + DoubleToString(lumoraPendingEntry, _Digits) + ",";
    payload += "\"expiry_price\":" + DoubleToString(expiryPrice, _Digits) + ",";
    payload += "\"stake_usd\":" + DoubleToString(stake, 2) + ",";
    payload += "\"payout_percent\":" + DoubleToString(payout, 2) + ",";
    payload += "\"profit_usd\":" + DoubleToString(resultPL, 2) + ",";
    payload += "\"expired_at\":\"" + LumoraJsonEscape(LumoraTimeText(TimeCurrent())) + "\"";
    payload += "}";

    LumoraPost("signal_result", payload);
